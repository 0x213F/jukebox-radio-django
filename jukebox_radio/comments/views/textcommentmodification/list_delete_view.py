...
class TextCommentModificationListDeleteView(BaseView, LoginRequiredMixin):
    def delete(self, request, **kwargs):
        ...
        text_comment_modification_qs = TextCommentModificatio.objects.filter(
            user=request.user, text_comment=text_comment
        )
        text_comment_modification_uuids = text_comment_modification_qs.values_list(
            "uuid", flat=True
        )
